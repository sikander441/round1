...
$score=0;
 ...
 $result=mysqli_query($conn,$sql);
 $row=mysqli_fetch_assoc($result);
 $p=$_SESSION['code_path'];
 if($row['c']==1){
   if($row['score']<$score)
   {
        $sql="update contest set score=$score,code='$p' where pid=$pid and teamname='$teamname'";
  ...
  $sql="insert into contest(teamname,pid,score,code) values('$teamname',$pid,$score,'$p')";
  mysqli_query($conn,$sql);
  $sql="update leaderboard set score=score+$score where teamname='$teamname'";
  mysqli_query($conn,$sql);
}
