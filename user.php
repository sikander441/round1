<?php  include('timer.php'); include('server.php');
...
<?php
  if(isset($_GET['teamname']))
  {
    ...
    $row=mysqli_fetch_assoc($result);
    $rank=$row['rank'];
    $sql="SELECT (select title from problems where pid=c.pid) as title,c.score,c.pid,c.time_min,c.time_sec,c.code from contest c where c.teamname='$teamname' order by c.pid";
    $result=mysqli_query($conn,$sql);
                       ...
                       <tbody>
                         ...
                         { $pid=$row['pid'];
                          ...
                          <tr>
                               ...
                               <td><?php echo $row['time_min']?>m:<?php echo $row['time_sec']?>s</td>
